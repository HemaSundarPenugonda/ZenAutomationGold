...
public class WebTable {
	...
	/**
	 * Search respective Column Label and value.
	 *
	 * @param columnLabel
	 *            the Column Label
	 * @param value
	 ...
	 */
	public boolean matchAgainstVerticalColumnLabel(String columnLabel,
			String value) {
		boolean flagkeyValuePair = false;
		List<WebElement> rows = webTable.findElements(By.tagName("tr"));
		List<WebElement> cols = new ArrayList<WebElement>();
		for (int iRowItterator = 0; iRowItterator < rows.size(); iRowItterator++) {
			cols.addAll(rows.get(iRowItterator).findElements(By.tagName("td")));
		}
		for (int iLabelValue = 0, label = 0, colValue = 1; iLabelValue < rows
				.size(); iLabelValue++, label = label + 2, colValue = colValue + 2) {
			if (cols.get(label).getText().equals(columnLabel)
					&& cols.get(colValue).getText().equals(value)) {
				flagkeyValuePair = true;
				break;
			}
		}

		return flagkeyValuePair;
	}
}
