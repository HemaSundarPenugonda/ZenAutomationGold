 ...
 *
 */
public class RegressionCreateProduct {
	...
	public RegressionCreateProduct(String srno, String runFlag,
		...
		if (pdPromoAllowed.equalsIgnoreCase("true")
			...
			this.pdPromoAllowed = Boolean.valueOf(pdPromoAllowed);
		} else {
			throw new SkipException(
		...
		if (pdEnrollmentOnly.equalsIgnoreCase("true")
			...
			this.pdEnrollmentOnly = Boolean.valueOf(pdEnrollmentOnly);
		} else {
			throw new SkipException(
		...
		if (pdAutoRenewOnly.equalsIgnoreCase("true")
			...
			this.pdAutoRenewOnly = Boolean.valueOf(pdAutoRenewOnly);
		} else {
			throw new SkipException(
	...
	/**
	 * @purpose:Initating Add product process.
	 */

	@Test(groups = { "napower" })
	public void testInitiateAddProduct() {
		Reporter.log("Exceution for Row number: " + this.pdSrno
				+ " Kicked off .");
		Logs.LOGGER.info("Exceution for Row number: " + this.pdSrno
				+ " Kicked off .");
		String sql = "select count(ProductId) as total from PRD.B_product where ProductCode='"
				+ this.pdCode + "'";
		ResultSet rs = db.getData(sql);
		Logs.LOGGER.info("Executed sql for product db verification as " + sql
				+ " .");
		try {
			if (rs.next()) {
				if (rs.getInt("total") > 0) {
					...
					Assert.fail("Product Code already exist please change it for Serial no:"
							+ this.pdSrno + " .");
				}
			}
		} catch (Exception e) {
		...
		if (testNavigateToAddProductPage()) {
			Reporter.log("testNavigateToAddProductPage passed");
		}

		if (testAddProduct()) {
			Reporter.log("testAddProduct passed");
		}
	 ...
	 *          navigated.
	 * @purpose:For verification of navigation in add product page.
	 */
	private boolean testNavigateToAddProductPage() {
		...
		if (Element.isTextPresent(browser, createProduct.getAddProductLabel(),
			...
			pass.append("Successfully navigated to add product page" + eol);
			ScreenShot.capture(true, "NavigateToAddProductPage");
			return true;
		} else {
			fail.append("Not able to navigate to add product page." + eol);
			return false;
		}
	...
	/**
	 * 
	 * @return: Boolean true-in case product created successfully/false-if not.
	 * @purpose:Fill information in Add product page and simultaneously validate
	 *               drop downs values.
	 */
	private boolean testAddProduct() {
		...
		if (this.pdCommissionType.equalsIgnoreCase("")) {
			...
			WebElement CommissionAmountElement = createProduct
					.getCommissionAmountElement();
			CommissionAmountElement.click();
			WebPage.waitForAngularRequestsToFinish(browser);
			...
			if (DropDown.isOptionTextPresentForAngularDropdown(
				...
				DropDown.selectAngularDropdownOptionByValue(
						...
						this.pdCommissionType);
				Logs.LOGGER.info("Option for Commission Type is selected as "
						+ this.pdCommissionType + " .");
				pass.append("Option for Commission Type is selected as "
				...
				if (this.pdCommissionAmount.equalsIgnoreCase("")) {
					...
					Element.isVisible(browser,
							createProduct.getCommissionAmountEditBox());
					createProduct.getCommissionAmountEditBox().sendKeys(
							this.pdCommissionAmount.toString());
				}
				trackPassFlag = true;
			} else {
				fail.append(this.pdCommissionType
						+ " Option for Commission Type in datasheet is not present in UI ."
						+ eol);
				trackPassFlag = false;
			}
		}
	...
	private boolean testValidateProductInfoInUI() {
		...
		try {
			if (createProduct
					.verifyErrorMessageAndClose("Product was not added.")) {
		...
		try {
			if (createProduct.informationMessageBox(this.pdCode + ", "
					+ this.pdName + " Successfully Added")) {
				ScreenShot.capture(true);
				createProduct.infoMessageClose();
				pass.append("Product added successfully with product code "
						+ this.pdCode + "." + eol);
				Logs.LOGGER
						.info("Product added successfully with product code "
								+ this.pdCode + ".");
				trackPassFlag = true;
			} else {
	...
	private boolean testValidateProductInfoInDataBase() {
		...
		try {
			...
			String sql = "select * from PRD.B_product p join [PRD].[B_ProductAccountType] pat "
					...
					+ this.pdDescription
					+ "' and p.ProductTagLine='" + this.pdTagLine + "'";
			ResultSet rs = db.getData(sql);
			if (rs.next()) {
				if (Integer.parseInt(rs.getString("ProductId")) > 1) {
					xlUpdate.updateExcelData(DataSource.globalConfig
							.get(Constants.CURRENT_TESTING + ".datasheet"),
							"AddProduct", rs.getString("ProductId"), Integer
