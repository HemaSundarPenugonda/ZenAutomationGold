...
import com.znalytics.enrollment.tx.api.util.GenerateDataForJson;
...
public class CreateJson {	
	...
	public static String JsonCustomerName;
	...
	public static void buildEnrollmentJson(int clStart, int clEnd, int baStart,
		...
		try {		    
			...
			FileInputStream fis = new FileInputStream(new File(Constants.DATAFILE));
		    //Constants.DATAFILE= Resources/testmodules/enrollment/Enrollment.xlsx
		    ...
		    String authContactEmailListIsPrimary = Enrollment.app.get("AuthorizedContactEmailList.IsPrimary");		     
		    String contactEmailEmailTypeName = Enrollment.app.get("EmailTypeNameLegal");
		    Boolean isPrimary = Boolean.valueOf(Enrollment.app.get("IsPrimary"));
		    String emailAddress = Enrollment.app.get("EmailAddress");
		    
		    String phoneTypeName = Enrollment.app.get("PhoneTypeName");
		    int phoneCountryCode = Integer.parseInt(Enrollment.app.get("PhoneCountryCode"));
		    int phoneAreaCode = Integer.parseInt(Enrollment.app.get("PhoneAreaCode"));
		    int phoneNumber = Integer.parseInt(Enrollment.app.get("PhoneNumber"));
		    int phoneExtension = Integer.parseInt(Enrollment.app.get("PhoneExtension"));
		    Boolean isSMSAllowed = Boolean.valueOf(Enrollment.app.get("IsSMSAllowed"));
		    Boolean isDoNotCall = Boolean.valueOf(Enrollment.app.get("IsDoNotCall"));
		    ...
		    String addressTypeNameLegal = Enrollment.app.get("AddressTypeNameLegal"); 
		    String attentionTo = Enrollment.app.get("AttentionTo"); 
		    String secondaryAttentionTo = Enrollment.app.get("SecondaryAttentionTo"); 
		    ...
		    UniqueCustomerName=Enrollment.app.get("CustomerName")+dateTime;
		    ContractNumber = Enrollment.app.get("ContractNumber")+dateTime;
		    ...
		    UtilityAccountNumber=Enrollment.app.get("UtilityAccountNumber")+dateTime;
		    IsOnHold= Boolean.valueOf(Enrollment.app.get("IsOnHold"));
		    
		    String depositAmountTypeName=null;
		    int depositAmount=0;		
		    
		    String companyName=Enrollment.app.get("CompanyName")+dateTime;
		    String dba=Enrollment.app.get("Dba")+ dateTime;
		    int taxId=Integer.parseInt(Enrollment.app.get("TaxId"));
		    
		    String driverLicenseStateProvince=Enrollment.app.get("DriverLicenseStateProvince");
		    String prefix=null;
		    String firstName=Enrollment.app.get("FirstName")+dateTime;
		    String middleName=Enrollment.app.get("MiddleName");
		    String lastName=Enrollment.app.get("LastName")+dateTime;
		    String suffix=null;
		    Boolean isDomesticViolence=Boolean.valueOf(Enrollment.app.get("IsDomesticViolence"));
		    Boolean isSenior=Boolean.valueOf(Enrollment.app.get("IsSenior"));
		    Boolean isEmployee=Boolean.valueOf(Enrollment.app.get("IsEmployee"));
		    String motherMaidenName=Enrollment.app.get("MotherMaidenName");
		    int last4SSN=Integer.parseInt(Enrollment.app.get("Last4SSN"));
		    int driverLicenseNumber=Integer.parseInt(Enrollment.app.get("DriverLicenseNumber"));
		     
		    GenerateDataForJson gs = new GenerateDataForJson();		    

		    /*for (int i = customerListRow; i <= customerListRow; i++) {
		    ...
		    for (int i = customerListRowStart; i <= customerListRowEnd; i++) {
			    ...
			    cEmailBA = new ContactEmail(contactEmailEmailTypeName, isPrimary, emailAddress);
			    aContactEmailListBA.add(new AuthorizedContactEmailList(isPrimary, cEmailBA));
			    cPhoneBA = new ContactPhone(phoneTypeName, isPrimary, phoneCountryCode, 
			    		 phoneAreaCode,  phoneNumber, phoneExtension, isSMSAllowed, isDoNotCall);
			    
			    aContactPhoneListBA.add(new AuthorizedContactPhoneList(
			    		isPrimary, cPhoneBA));
			    aContactListBA.add(new AuthorizedContactList(contactTypeName,
			    		contactName, birthDate, isPrimary, isSMSAllowed, 
			    		aContactPhoneListBA, aContactEmailListBA));
			    contractList.add(gs.generateDataForContractList(
				    sheetBillingAccount, j, 8));
			    mailingAdd = new MailAddress(addressTypeName,
			    		attentionTo, address1, address2,
			    		Address3, city, County, state, zip5, zip4,country);
			    secondaryNotificationAdd = new SecondaryNotificationAddress(
			    		addressTypeName, secondaryAttentionTo, address1, address2,
			    		Address3, city, County, state, zip5, zip4, country);
			    ...
			    for (int k = serviceAcRowStart; k <= serviceAcRowEnd; k++) {
					...
					cEmailSA = new ContactEmail(contactEmailEmailTypeName, isPrimary, emailAddress);
					aContactEmailListSA.add(new AuthorizedContactEmailList(
							isPrimary, cEmailSA));
					cPhoneSA = new ContactPhone(phoneTypeName, isPrimary, phoneCountryCode, phoneAreaCode, phoneNumber,
				    		phoneExtension, isSMSAllowed,isDoNotCall);
					aContactPhoneListSA.add(new AuthorizedContactPhoneList(
							isPrimary, cPhoneSA));
					aContactListSA.add(new AuthorizedContactList(contactTypeName,
				    		contactName, birthDate, isPrimary, isSMSAllowed, 
				    		aContactPhoneListSA, aContactEmailListSA));
					...
					for (int l = contractRateSegmentListRowStart; l <= contractRateSegmentListRowEnd; l++) {
					    ...
					    contractRateSegmentList.add(gs.generateDataForContractRateSegmentList(
					...
					contractRateSchedule = gs.generateDataForContractRateSchedule(sheetServiceAccount, k, 0,contractRateSegmentList);
	
					contractDetailList.add(new ContractDetailList(ContractNumber, ContractStartDate, ContractEndDate, 
							depositAmountTypeName, depositAmount, contractRateSchedule));
					...
					for (int m = meterListRowStart; m <= meterListRowEnd; m++) {						
					    ...
					    meterOwnerEntity.add(gs.generateDataForMeterOwnerEntity(
							    sheetMeterList, m, 6));
					...
					splNeedsAddress = new SpecialNeedsAddress(addressTypeName,
				    		attentionTo, address1, address2,
				    		Address3, city, County, state, zip5, zip4,country);
					survey.add(new Survey(surveyQuestionNumber, surveyResponse));
			...
			cEmailCL = new ContactEmail(contactEmailEmailTypeName,isPrimary, emailAddress);
			aContactEmailListCL.add(new AuthorizedContactEmailList(isPrimary,
				cEmailCL));
			cPhoneCL = new ContactPhone(phoneTypeName, isPrimary, phoneCountryCode, 
		    		phoneAreaCode, phoneNumber,
		    		phoneExtension, isSMSAllowed, isDoNotCall);
			aContactPhoneListCL.add(new AuthorizedContactPhoneList(isPrimary,
				cPhoneCL));
			aContactListCL.add(new AuthorizedContactList(contactTypeName,
		    		contactName, birthDate, isPrimary, 
		    		isSMSAllowed, aContactPhoneListCL, aContactEmailListCL));
			legalAdd = new LegalAddress(attentionTo, addressTypeNameLegal, address1, address2,
		    		Address3, city, County, state, zip5, zip4,country);
			custIndividual = new CustomerIndividual(driverLicenseStateProvince, prefix, firstName,
				middleName, lastName, suffix, isDomesticViolence, isSenior, isEmployee, motherMaidenName, last4SSN,
				birthDate, driverLicenseNumber);
			custCommercial = new CustomerCommercial(companyName,
				dba, taxId);
			...
			JsonFilePath= System.getProperty("user.dir")+ "/Resources/Testmodules/enrollment/Json_Generated/"+ "Enrollment"+ dateTime+ ".json";
			...
			try {
			    ...
			    //FileWriter writer = new FileWriter(System.getProperty("user.dir")+"/Resources/Testmodules/enrollment/Json_Generated/Enrollment.json");
