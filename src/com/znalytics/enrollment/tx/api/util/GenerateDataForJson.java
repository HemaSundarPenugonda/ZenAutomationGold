...
public class GenerateDataForJson {
	...
	// String cName;
	try {
	    System.out.println("GENERATING CUSTOMERLIST in JSON");
		...
		System.out.println("Cname not equal to BLANK");
		customerList.setCustomerName(cName);
		CreateJson.JsonCustomerName = cName;
	    } else {
		System.out.println("Cname is equal to BLANK");
		customerList.setCustomerName(CreateJson.UniqueCustomerName);
		CreateJson.JsonCustomerName = CreateJson.UniqueCustomerName;
	    }

	    // customerList.setCustomerName(CreateJson.UniqueCustomerName);
	    // CreateJson.JsonCustomerName = CreateJson.UniqueCustomerName;
	    // cName="";
	    customerList.setPassCode(getStringValue(sheet, row, col = col + 2));
	...
	try {
	    /*
	     * String prodCode =getStringValue(sheet, row, col); if
	     * (!prodCode.equals(null)) {
	     * System.out.println("prodCode is not null");
	     * //customerList.setCustomerName(getStringValue(sheet, row, col));
	     * } else { System.out.println("prodCode is null");
	     * //customerList.setCustomerName("ZAutoCust_dateTimeStamp"); }
	     * contractRateSchedule.setProductCode(prodCode);
	     */
