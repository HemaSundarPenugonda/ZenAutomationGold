...
public class Enrollment {
	...
	public Enrollment(){
		...
		System.out.println("Constants.CURRENT_TESTING= "+Constants.CURRENT_TESTING);
		Enrollment.app = TxtDataEngine
				.readTxtFile(DataSource.globalConfig
						.get(Constants.CURRENT_TESTING + ".properties"));
		System.out.println("Enrollment.app= "+Enrollment.app);
		Enrollment.css = TxtDataEngine
				.readTxtFile(DataSource.globalConfig
	...
	public void loadData() {
		...
		System.out.println(Constants.CURRENT_TESTING + " test type: " + testType);
		try {
			if (this.testType.toLowerCase().equals("smoke")) {
				...
				System.out.println("moduleToRun= "+moduleToRun);
	...
	public void generateRegTestNGXML(String testType, List<String> scenarioName) {
		...
		try {
			...
			System.out.println("Inside generateRegTestNGXML method");
			if (Utils.containsIgnoreCase(scenarioName, "EnrollNewCommercialCustomer")) {
				System.out.println("Testing EnrollNewCommercialCustomer Scenario");
				XmlTest regressionTestEnrollNewCommercialCustomer = new XmlTest(suiteRegression);
				System.out.println("IF EnrollNewCommercialCustomer CONDITION is true");
				// Add Parameter
				regressionTestEnrollNewCommercialCustomer.setPreserveOrder("true");
				regressionTestEnrollNewCommercialCustomer.setVerbose(1);
				// Add listener
				...
				// Define test node
				regressionTestEnrollNewCommercialCustomer
				...
				regressionTestEnrollNewCommercialCustomer.setXmlClasses(classes);

				// Set the Groups
				List<String> groups = new ArrayList<String>();
				groups.add(Constants.CURRENT_CLIENT);
				regressionTestEnrollNewCommercialCustomer.setIncludedGroups(groups);
			}
			/*if (Utils.containsIgnoreCase(scenarioName, "EnrollNewResidentialCustomer")) {
				System.out.println("Testing EnrollNewResidentialCustomer Scenario");
				XmlTest regressionTestEnrollNewResidentialCustomer = new XmlTest(suiteRegression);
				System.out.println("IF EnrollNewResidentialCustomer CONDITION is true");
				// Add Parameter
				regressionTestEnrollNewResidentialCustomer.setPreserveOrder("true");
				regressionTestEnrollNewResidentialCustomer.setVerbose(1);
				// Add listener
				List<String> listnerClasses = new ArrayList<String>();
				listnerClasses
						.add("com.znalytics.framework.Listener.MethodInterceptorListener");
				...
				// Define test node
				regressionTestEnrollNewResidentialCustomer
						.setName("Znalytics Execution Report - "
								+ Constants.CURRENT_TESTING + " "
								+ this.testType.toLowerCase()
								+ " Enrollment API Test");
				...
				regressionTestEnrollNewResidentialCustomer.setXmlClasses(classes);

				// Set the Groups
				List<String> groups = new ArrayList<String>();
				groups.add(Constants.CURRENT_CLIENT);
				regressionTestEnrollNewResidentialCustomer.setIncludedGroups(groups);
			}
*/
